"use client";
import { Environment, useScroll, Preload } from "@react-three/drei";
...
export default function Experience() {
  ...
  useFrame(() => {
    ...
    const lookAtX =
      current.lookAt[0] + (next.lookAt[0] - current.lookAt[0]) * progress;
    const lookAtY =
      current.lookAt[1] + (next.lookAt[1] - current.lookAt[1]) * progress;
    const lookAtZ =
      current.lookAt[2] + (next.lookAt[2] - current.lookAt[2]) * progress;
